#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 3/25/2023 2:06 AM
# @Author  : Guofeng Mei
# @Email   : Guofeng.Mei@student.uts.edu.au
# @File    : indoor3d_util.py
# @Software: PyCharm
import numpy as np
import glob
import os
...
g_classes = [x.rstrip() for x in open(os.path.join(BASE_DIR, '../metas/class_names.txt'))]
...
def room2blocks(root, data_label_filename, data, label, num_point, block_size=1.0, stride=1.0,
    ...
    if not os.path.exists(os.path.join(root, "indoor3d_sem_seg_hdf5_data_test/raw_data3d")):
        os.makedirs(os.path.join(root, "indoor3d_sem_seg_hdf5_data_test/raw_data3d"))
    if not os.path.exists(os.path.join(root, "indoor3d_sem_seg_hdf5_data_test/raw_data3d/Area_" + str(test_area))):
        os.makedirs(os.path.join(root, "indoor3d_sem_seg_hdf5_data_test/raw_data3d/Area_" + str(test_area)))
    # Collect blocks
    block_data_list = []
    block_label_list = []
    global raw_data_index
    for idx in range(len(xbeg_list)):
        ...
        w_path = os.path.join(root, 'indoor3d_sem_seg_hdf5_data_test/raw_data3d/Area_'
                              + str(test_area) + '/' + str(room_name) + '(' + str(raw_data_index) + ').txt')
...
def room2blocks_plus(root, data_label, num_point, block_size, stride,
    ...
    return room2blocks(root, data, label, num_point, block_size, stride,
                       random_sample, sample_num, sample_aug)


def room2blocks_wrapper(root, data_label_filename, num_point, block_size=1.0, stride=1.0,
                        random_sample=False, sample_num=None, sample_aug=1):
    if data_label_filename[-3:] == 'txt':
        ...
        exit()
    return room2blocks_plus(root, data_label, num_point, block_size, stride,
                            random_sample, sample_num, sample_aug)


def room2blocks_plus_normalized(root, data_label_filename, data_label, num_point, block_size, stride,
                                random_sample, sample_num, sample_aug):
    """ room2block, with input filename and RGB preprocessing.
    ...
    data_batch, label_batch = room2blocks(root, data_label_filename, data, label, num_point, block_size, stride,
                                          random_sample, sample_num, sample_aug)
...
def room2blocks_wrapper_normalized(root, data_label_filename, num_point, block_size=1.0, stride=1.0,
                                   random_sample=False, sample_num=None, sample_aug=1):
    if data_label_filename[-3:] == 'txt':
        data_label = np.loadtxt(data_label_filename)
    elif data_label_filename[-3:] == 'npy':
        ...
        print('Unknown file type! exiting.')
        exit()
    return room2blocks_plus_normalized(root, data_label_filename, data_label, num_point, block_size, stride,
                                       random_sample, sample_num, sample_aug)
