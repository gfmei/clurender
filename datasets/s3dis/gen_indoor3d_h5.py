#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 3/25/2023 2:04 AM
# @Author  : Guofeng Mei
# @Email   : Guofeng.Mei@student.uts.edu.au
# @File    : gen_indoor3d_h5.py
# @Software: PyCharm
import os
import sys
...
from datasets.s3dis import data_prep_util
...
# Set paths
filelist = os.path.join(BASE_DIR, '../metas/all_data_label.txt')
...
for i, data_label_filename in enumerate(data_label_files):
    ...
    data, label = indoor3d_util.room2blocks_wrapper_normalized(ROOT_DIR, data_label_filename, NUM_POINT, block_size=1.0,
                                                               stride=1, random_sample=False, sample_num=None)
    print('{0}, {1}'.format(data.shape, label.shape))
    for xxx in range(data.shape[0]):
