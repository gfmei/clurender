...
from datasets.datautil import pc_normalize, points_sampler
...
def load_data_partseg(root, partition):
    ...
    if partition == ['train', 'val']:
        file = (glob.glob(os.path.join(root, 'shapenet_part_seg_hdf5_data', '*train*.h5')) +
                glob.glob(os.path.join(root, 'shapenet_part_seg_hdf5_data', '*val*.h5')))
    else:
        file = glob.glob(os.path.join(root, 'shapenet_part_seg_hdf5_data', '*%s*.h5' % partition))
    for h5_name in file:
        f = h5py.File(h5_name, 'r+')
        data = f['data'][:].astype('float32')
        label = f['label'][:].astype('int64')
...
def prepare_test_data_semseg(root):
    if not os.path.exists(os.path.join(root, 'stanford_indoor3d')):
        os.system('python prepare_data/collect_indoor3d_data.py')
    if not os.path.exists(os.path.join(root, 'indoor3d_sem_seg_hdf5_data_test')):
        os.system('python prepare_data/gen_indoor3d_h5.py')


def load_data_semseg(root, partition, test_area):
    if partition == 'train':
        data_dir = os.path.join(root, 'indoor3d_sem_seg_hdf5_data')
    else:
        data_dir = os.path.join(root, 'indoor3d_sem_seg_hdf5_data_test')
    with open(os.path.join(data_dir, "all_files.txt"), 'r', encoding='utf-8', errors='ignore') as f:
        all_files = [line.rstrip() for line in f]
    with open(os.path.join(data_dir, "room_filelist.txt"), 'r', encoding='utf-8', errors='ignore') as f:
        room_filelist = [line.rstrip() for line in f]
    data_batchlist, label_batchlist = [], []
    for f in all_files:
        file = h5py.File(os.path.join(root, f), 'r')
        data = file["data"][:]
        label = file["label"][:]
...
def load_color_partseg():
    ...
    with open(os.path.join(root, "metas/partseg_colors.txt"), 'r', encoding='utf-8', errors='ignore') as f:
        for line in json.load(f, strict=False):
...
def load_color_semseg():
    ...
    labels = []
    root = os.path.dirname(os.path.abspath(__file__))
    with open(os.path.join(root, "metas/semseg_colors.txt"), 'r', encoding='utf-8', errors='ignore') as f:
        for line in json.load(f, strict=False):
            colors.append(line['color'])
            labels.append(line['label'])
        ...
        for _ in range(0, img_size):
            ...
            for _ in range(0, img_size):
                ...
                label_index = label_index + 1
                if color_index >= 13:
                    cv2.imwrite("metas/semseg_colors.png", img, [cv2.IMWRITE_PNG_COMPRESSION, 0])
                    return np.array(colors)
...
class ShapeNetPart(Dataset):
    def __init__(self, root, num_points, partition='train', class_choice=None):
...
class S3DIS(Dataset):
    def __init__(self, root, num_points=4096, partition='train', test_area='5'):
